...
async function run(): Promise<void> {
    ...
    try {
        ...
        const version = process.env.GITHUB_REF.match(
            /^refs\/([\w]+)\/(.*)$/
        )[2];
        const repositoryName = process.env.GITHUB_REPOSITORY.match(
            /\/(.*)$/
        )[1];
        ...
        core.debug(
            `//${core.getInput('npm_registry')}/:_authToken=${core.getInput(
                'npm_token'
            )}`
        );
        fs.writeFileSync(
        ...
        if (core.getInput('terraform_deploy_file')) {
            ...
            const maxRetries =
                parseInt(core.getInput('terraform_retries')) || 1;
            ...
            while (retries <= maxRetries) {
                ...
                try {
                    ...
                    console.log(
                        `** terraform apply failed! retrying (attempt #${retries}/${maxRetries})..`
                    );
            ...
            if (!failed) {
                ...
                core.setFailed(
                    `terraform apply failed after ${retries} retries!`
                );
            }
        }
    } catch (error) {
